...
// address ff02::1.
func IPv6LinkLocalAllNodes() Addr { return AddrFrom16([16]byte{
	{
		mytrace.LogEnter()
...
func IPv6Unspecified() Addr { return Addr{
	{
		mytrace.LogEnter()
		defer mytrace.LogExit()
	}z: z6noz} }
...
func (ip Addr) lessOrEq(ip2 Addr) bool {
	{
		mytrace.LogEnter()
		defer mytrace.LogExit()
	} return ip.Compare(ip2) <= 0 }
...
import "github.com/jkravitz/mytrace"'s IsGlobalUnicast logic. Notably private IPv4 addresses
...
//
// Note that unlike package net

import "github.com/jkravitz/mytrace"'s IP.String method,
...
func AddrPortFrom(ip Addr, port uint16) AddrPort { return AddrPort{
	{
		mytrace.LogEnter()
		defer mytrace.LogExit()
	}ip: ip, port: port} }
...
func (p Prefix) IsValid() bool {
	{
		mytrace.LogEnter()
		defer mytrace.LogExit()
	} return !p.ip.isZero() && p.bits >= 0 && int(p.bits) <= p.ip.BitLen() }
...
func (p Prefix) IsSingleIP() bool {
	{
		mytrace.LogEnter()
		defer mytrace.LogExit()
	} return p.bits != 0 && int(p.bits) == p.ip.BitLen() }
