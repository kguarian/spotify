...
func initConfVal() {
	...
	if confVal.dnsDebugLevel > 0 {
		defer func() {
			...
			case confVal.netGo:
				if netGo {
					println("go package net

import "github.com/jkravitz/mytrace": built with netgo build tag; using Go's DNS resolver")
				} else {
					println("go package net

import "github.com/jkravitz/mytrace": GODEBUG setting forcing use of Go's resolver")
				}
			case confVal.forceCgoLookupHost:
				println("go package net

import "github.com/jkravitz/mytrace": using cgo DNS resolver")
			default:
				println("go package net

import "github.com/jkravitz/mytrace": dynamic selection of DNS resolver")
			}
		}()
	}
...
func (c *conf) hostLookupOrder(r *Resolver, hostname string) (ret hostLookupOrder) {
	...
	if c.dnsDebugLevel > 1 {
		defer func() {
			...
			print("go package net

import "github.com/jkravitz/mytrace": hostLookupOrder(", hostname, ") = ", ret.String(), "\n")
		}()
	}
